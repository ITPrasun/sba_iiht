...
namespace FSE_ProjMgr.Log
{
    public class LogTransactionException : ITraceWriter
    {
            ...
            if (level != TraceLevel.Off)
            {
                if (traceAction != null && traceAction.Target != null)
                {
                    if (string.IsNullOrEmpty(category))
                        ...
                        category = category + "," + traceAction.Target.ToString();
                }
        ...
        private void Log(TraceRecord record)
        {
            ...
            if (!string.IsNullOrWhiteSpace(record.Message))
                message.Append("Operation Result Format: ").Append(record.Message + " , ");
