...
export class AddTaskComponent implements OnInit {
  ...
  constructor(private eventService: EventService, private projectService: ProjectService,
    ...
    if (route.snapshot.params['task']) {
      ...
      this.taskToAdd.StartDate = this.taskToAdd.StartDate ?
        moment(this.taskToAdd.StartDate).format('MM-DD-YYYY').toString() : moment(new Date()).format('MM-DD-YYYY').toString();
      this.taskToAdd.EndDate = this.taskToAdd.EndDate ? moment(this.taskToAdd.EndDate).format('MM-DD-YYYY').toString() :
      ...
      this.taskToAdd.StartDate = moment(new Date()).format('MM-DD-YYYY').toString();
      this.taskToAdd.EndDate = moment(new Date()).add(1, 'days').format('MM-DD-YYYY').toString();
  ...
  setMinEndDate($event) {
    this.minEndDate = moment(this.taskToAdd.StartDate).add(1, 'days').toDate();
    if (moment(this.taskToAdd.EndDate) <= moment(this.taskToAdd.StartDate)) {
      this.taskToAdd.EndDate = moment(this.minEndDate).format('MM-DD-YYYY').toString();
    }
  }

  addTask() {
    debugger;
    if (!this.taskToAdd.ProjectID && this.buttonName == 'Add') {
      this.eventService.showWarning('Please select project ');
      return;
    }
    if (!this.taskToAdd.TaskName || this.taskToAdd.TaskName === '') {
    ...
    if (!this.hasParentTask && (!this.taskToAdd.StartDate || this.taskToAdd.StartDate.toString() === '')) {
      this.eventService.showWarning('Please select start date ');
      return;
    }
    if (!this.hasParentTask && (!this.taskToAdd.EndDate || this.taskToAdd.EndDate.toString() === '')) {
    ...
    if (type === 2) {
      if (!this.hasParentTask) {
        ...
        this.taskService.getParentTask().subscribe((parentTask) => {
          ...
          alert(this.parentTasks[0].parent_Task_Name);
          this.modalRef = this.modalService.show(template);
          document.getElementsByTagName("modal-container")[0].classList.remove("fade");
          this.eventService.showLoading(false);
        },
  ...
  selectProj() {
    this.taskToAdd.ProjectID = +this.projects[this.selectedIndex].projectId;
  ...
  selectParentTask() {
    debugger;
    this.taskToAdd.ParentID = +this.parentTasks[this.selectedIndexParent].parent_ID;
    this.selectedParentTask = this.parentTasks[this.selectedIndexParent].parent_Task_Name;
    this.selectedIndexParent = null;
    this.modalRef.hide();
  }
  selectUser() {
  ...
  hasParTaskChange($event) {
    if (this.hasParentTask) {
      ...
      this.taskToAdd.StartDate = null;
      this.taskToAdd.EndDate = null;
